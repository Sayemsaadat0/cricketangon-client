        ...
        <div>
            <div className="relative rounded-[10px]">
                <input
                    ...
                    type="text"
                    id={id}
                    className={` ${error && 'border-red-500'} ${cn(inputVariants({ variant, className }))}`}
                    {...props}
                    placeholder={placeholder ? placeholder : ''}
                />
                <label
                    htmlFor={id}
                    className={`absolute rounded-[15px] text-sm transform bg-white peer-focus:bg-white placeholder-black cursor-pointer
